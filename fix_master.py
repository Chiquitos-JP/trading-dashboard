...
# 検証：SOFIとIONQのみが信用オープンであることを確認
print("\n=== オープンポジション検証 ===")
for ticker in ['SOFI', 'IONQ', 'CRWV', 'KTOS', 'IOT', 'PLTR']:
    t_df = df[df['ticker'] == ticker]
    if len(t_df) > 0:
        buy = t_df[t_df['trade_type'] == '買建']['quantity'].sum()
        sell = t_df[t_df['trade_type'] == '売埋']['quantity'].sum()
        net = buy - sell
        if net != 0:
            print(f"  {ticker}: 買建{buy:,.0f} - 売埋{sell:,.0f} = {net:,.0f}")
        else:
            print(f"  {ticker}: 決済済み")

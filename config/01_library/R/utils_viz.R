...
# ### B-3-3. plot_helpers.Rの読込 ----
# helpers_path <- file.path(Sys.getenv("PROJECT_ROOT"), "00_config/plot_helpers.R")
...
## Key Insights（被り対策版：1つのリッチテキストで描画） 
# make_key_insights_panel <- function(ts_monthly,
#                                     family = "roboto",
#                                     bg_fill = "#f3f3f3",
#                                     title_color = "#222222",
#                                     text_color  = "#222222",
...
#                                     lineheight = 1.15,
#                                     left = 0.02, top = 0.90,
#                                     avg_on_newline = TRUE   # ★ 追加: TRUE=改行, FALSE=1行
# ) {
#   stopifnot(all(c("ttl_gain_realized_jpy",
#                   "num_of_trades",
#                   "num_of_win_trades",
#                   "actual_trade_days",
#                   "year_month") %in% names(ts_monthly)))
#   
#   # --- 指標 ---
#   ytd    <- sum(ts_monthly$ttl_gain_realized_jpy, na.rm = TRUE)
#   wins   <- sum(ts_monthly$num_of_win_trades,    na.rm = TRUE)
#   trades <- sum(ts_monthly$num_of_trades,        na.rm = TRUE)
#   wr     <- if (trades > 0) wins / trades else NA_real_
#   tot_days <- sum(ts_monthly$actual_trade_days, na.rm = TRUE)
#   avg_day  <- if (tot_days > 0) ytd / tot_days else NA_real_
#   
#   # 区切り（改行 or 中黒）
#   sep <- if (avg_on_newline) "<br>" else "&nbsp;•&nbsp;"
#   
#   # HTML文字列を生成、計算値が実際の意味と一致しているか確認する必要性あり
#   key_html <- glue::glue(
#     "<span style='font-weight:700; color:{title_color}'>Key Insights</span><br>
#      <span style='font-weight:600'>YTD cumulative:</span> {scales::comma(ytd)} JPY<br>
#      <span style='font-weight:600'>YTD Win rate:</span> {scales::percent(wr, accuracy = 0.1)}{sep}
#      <span style='font-weight:600'>YTD Avg/day:</span> {scales::comma(round(avg_day))} JPY"
#   )
#   
#   ggplot() +
...
#     ggtext::geom_richtext(
#       data = tibble::tibble(x = left, y = top, label = key_html),
#       aes(x, y, label = label),
#       hjust = 0, vjust = 1,
#       label.colour = NA, fill = NA,
#       family = family,
...
## A-4. Caption ---- 
## クレジット（キャプション）変数
# current_year_for_caption <- format(Sys.Date(), "%Y")
# # author_credit <- paste0("M.Ogata © ", current_year_for_caption)
# author_credit <- paste0("M.Ogata © ", current_year_for_caption)
# source_credit <- "Source: SBI, Rakuten"

## キャプション生成関数
# make_caption <- function(author = author_credit, src = source_credit) {
#   paste(author, "|", src)
# }

.current_year <- function() format(Sys.Date(), "%Y")
.default_author <- function() paste0("Casual-Investment©", .current_year())
# .default_author <- function() paste0("M.Ogata © ", .current_year())

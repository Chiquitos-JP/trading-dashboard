...
#
# 連続スケールの X/Y が「生の大きな数値」（金額・人口など）のときは、軸ラベルを必ず短縮単位で整形する。
# X 軸ラベルが画面幅を超えると重なる。大きな金額・人口は必ず短縮単位 (K/M/B/T) で整形すること。
# scales::label_number(scale_cut = scales::cut_short_scale()) が定番。
# 例:
# scale_x_continuous(
#   labels = scales::label_number(scale_cut = scales::cut_short_scale(), prefix = "$"),
...
<!--
レンダリング前の自己チェック:
- [ ] 数値軸ラベルが raw 値（カンマ区切りのみ）になっていないか
- [ ] 軸ラベル幅が画面幅を超えていないか（特に X 軸の T/B/M 単位）
- [ ] 凡例の長文（国名フルネーム等）が枠外に押し出されていないか
-->

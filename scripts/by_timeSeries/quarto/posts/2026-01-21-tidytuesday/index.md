...
if (!dir.exists(file.path(base_path, "data"))) {
  base_path <- dirname(dirname(dirname(base_path)))
}

# Load realized P/L data
pl_path <- file.path(base_path, "data/trading_account/realized_pl/silver/realized_pl.parquet")

if (file.exists(pl_path)) {
  df <- read_parquet(pl_path) %>%
    mutate(settlement_date = as.Date(settlement_date))
  cat("Loaded", nrow(df), "trading records\n")
  cat("Date range:", as.character(min(df$settlement_date)), "to", as.character(max(df$settlement_date)), "\n")
} else {
...
```
## Loaded 1426 trading records
## Date range: 2021-02-22 to 2026-01-16
```

``` r
df %>%
  select(settlement_date, ticker, profit_jpy) %>%
  head()
```

```
## # A tibble: 6 × 3
##   settlement_date ticker profit_jpy
##   <date>          <chr>       <dbl>
## 1 2021-02-22      4936        59825
## 2 2021-03-02      4175       431465
## 3 2021-09-21      BMY        -74794
## 4 2021-09-24      XOM        -62818
## 5 2021-11-25      4452       -34035
## 6 2021-12-02      2229        13167
```
...
![plot of chunk calendar-2025](figure/calendar-2025-1.png)
...
![plot of chunk calendar-2026](figure/calendar-2026-1.png)
...
|month |Total P/L   | Trading Days|Avg P/L   | Profitable Days|Win Rate |
|:-----|:-----------|------------:|:---------|---------------:|:--------|
|1月   |¥14,432     |            4|¥3,608    |               3|75%      |
|2月   |-¥46,363    |            8|-¥5,795   |               6|75%      |
|3月   |-¥3,282,961 |           10|-¥328,296 |               4|40%      |
|5月   |¥1,811,121  |           11|¥164,647  |               9|81.8%    |
|6月   |¥1,823,545  |           12|¥151,962  |              12|100%     |
|7月   |¥533,590    |            5|¥106,718  |               4|80%      |
|8月   |¥3,253,378  |           13|¥250,260  |              10|76.9%    |
|9月   |¥2,643,828  |           12|¥220,319  |              11|91.7%    |
|10月  |¥5,055,854  |           16|¥315,991  |              14|87.5%    |
|11月  |¥2,780,676  |            8|¥347,584  |               8|100%     |
|12月  |¥1,112,490  |            4|¥278,122  |               4|100%     |
...
```
## Worst Day: 2024-08-07 ( -¥3,478,929 )
```
...
```
## Average Profit (winning days): ¥301,577
```
...
```
## Average Loss (losing days): -¥373,637
```

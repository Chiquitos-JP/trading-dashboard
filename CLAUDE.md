...
- Quarto: ローカルインストール済み
- R: 未インストール（TidyTuesday はローカルレンダリング不可）
- ストレージ: Dropbox 上（`WinError 32` は無視可 — Dropbox ロック由来）

### Mac 環境

- OS: macOS 26.3 (Tahoe) ARM64 (Apple Silicon M1)
...
- R: 4.3.1 インストール済み（TidyTuesday のローカルレンダリングが潜在的に可能）
...
- **ローカルレンダリング不可**（Windows ARM64 + R knitr 非互換）

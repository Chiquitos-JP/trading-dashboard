...
- プロジェクトルートの **`.playwright-cli/`** に CLI・ブラウザ関連のローカル専用ファイルを置く（Windows 本機の絶対パス例: `C:\Users\alpac\Dropbox\03_individual_work\05_stockTrading\.playwright-cli`）
- `.gitignore` で除外（コミットしない）。詳細は `.cursor/rules/environment.mdc` の「Playwright（ローカル）」
...
部分実行スクリプト:
- `run_weekly_review.py` — 今週分 raw CSV チェック（楽天/SBI 個別、日曜 0:00 JST 起点）→ 不足あれば `scripts/broker_download/fetch.py --broker {name}` で半自動取得 → 外部マーケット・マクロ・カレンダー更新 → Weekly Review + Quarto + Dashboard（`market_data_visualization` は `--force-update`）。環境変数: `SKIP_TRADING_RAW_FETCH=1` で取得スキップ、`TRADING_RAW_FETCH_SCRIPT=<path>` でパス上書き
...
`scripts/broker_download/fetch.py` で楽天証券・SBI証券からの CSV ダウンロードを半自動化。
Playwright CLI（`@playwright/cli`）を使用。

### 実行方法

| 操作 | Windows | Mac |
|---|---|---|
| 楽天のみ | `py scripts/broker_download/fetch.py --broker rakuten` | `python3 scripts/broker_download/fetch.py --broker rakuten` |
| SBI のみ | `py scripts/broker_download/fetch.py --broker sbi` | `python3 scripts/broker_download/fetch.py --broker sbi` |
| 全証券会社 | `py scripts/broker_download/fetch.py --broker all` | `python3 scripts/broker_download/fetch.py --broker all` |
| 新規ログイン | `--fresh-login` オプション追加 | 同左 |
| 楽天ブラウザ閉 | `playwright-cli -s=broker close` | 同左 |
| SBI ブラウザ閉 | `playwright-cli -s=sbi close` | 同左 |

### フロー（ポーリング方式 — ターミナル Enter 不要）

認証完了を `playwright-cli snapshot` で自動検出するため、Cursor 等 stdin が繋がらない環境でも動作する。ダウンロード完了後はブラウザを自動クローズする（例外時も `try/finally` で必ず閉じる）。

**楽天証券:**
1. コマンド実行 → ブラウザ自動起動 → ID/PW 自動入力 → ログイン送信
2. ユーザーがブラウザ上で 2段階認証を完了（SMS / メール / アプリ）
3. 自動: ログイン完了をポーリング検出 → セッション保存 → 実現損益(国内/米国) + 取引履歴(国内/米国) の CSV 4件をダウンロード → ブラウザ自動クローズ

**SBI証券:**
1. コマンド実行 → ブラウザ自動起動 → ID/PW 自動入力 → パスワードログイン送信
2. 自動: デバイス認証ページで「Eメールを送信する」をクリック
3. ユーザーがメールの認証URLを開いてデバイス登録を完了（ブラウザ側で登録完了が反映される）
4. 自動: ログイン完了をポーリング検出 → 外国株式 約定履歴 + 信用決済明細 の CSV 2件をダウンロード → ブラウザ自動クローズ

### 環境変数

- `BROKER_LOGIN_WAIT_SEC` — ログイン完了検出のタイムアウト秒数（デフォルト 900 秒、最小 60 秒）。2段階認証に時間がかかる場合は延長可。
- `RAKUTEN_LOGIN_ID` / `RAKUTEN_LOGIN_PW` / `SBI_LOGIN_ID` / `SBI_LOGIN_PW` — `.env` に設定（必須）

### セッション

- `data/broker_sessions/rakuten-auth.json` — 楽天（セッション名: `broker`）
- `data/broker_sessions/sbi-auth.json` — SBI（セッション名: `sbi`）
- 有効期限内はログイン・認証をスキップして全自動実行

### 前提

- `npm install -g @playwright/cli@latest` が必要（Node.js 18+）
- `.env` に `RAKUTEN_LOGIN_ID` / `RAKUTEN_LOGIN_PW` / `SBI_LOGIN_ID` / `SBI_LOGIN_PW` を設定

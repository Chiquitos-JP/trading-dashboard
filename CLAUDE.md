...
2. ユーザーが 2段階認証を完了 → ターミナルで Enter
3. 自動: 実現損益(国内/米国) + 取引履歴(国内/米国) の CSV 4件をダウンロード

**SBI証券:**
1. コマンド実行 → ブラウザ自動起動 → ID/PW 自動入力 → パスワードログイン送信
2. 自動: デバイス認証ページで「Eメールを送信する」をクリック
3. ユーザーがメールの認証URLを開いてデバイス登録を完了 → ターミナルで Enter
4. 自動: 外国株式 約定履歴 + 信用決済明細 の CSV 2件をダウンロード
